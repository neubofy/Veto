...
object GoogleDriveUploader {
    ...
    private fun releaseToPublicDownloads(context: Context, file: File, type: String) {
        try {
            val fileName = "Veto_${type.uppercase()}_${file.name}"
            ...
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q) {
                val contentValues = android.content.ContentValues().apply {
                    ...
                    put(android.provider.MediaStore.MediaColumns.RELATIVE_PATH, android.os.Environment.DIRECTORY_DOWNLOADS + "/Veto")
                }
                ...
                if (uri != null) {
                    ...
                    context.log().i(TAG, "Uploaded failed; released file to Public Downloads via MediaStore: $uri")
                    return
                }
            }
            ...
            context.log().i(TAG, "Uploaded failed; released file to Public Downloads: ${destFile.absolutePath}")
        } catch (e: Exception) {
            context.log().e(TAG, "Failed to release file to Public Downloads: ${e.message}")
        }
    }
    ...
    fun uploadFile(context: Context, file: File, mimeType: String, type: String, onSuccess: (String) -> Unit, onError: (String) -> Unit) {
        Thread {
            ...
            try {
                ...
                if (!uploadSuccessful && file.exists()) {
                    releaseToPublicDownloads(context, file, type)
                }
                // ALWAYS clean up the temporary app cache file!
                if (file.exists()) {
                    try {
                        file.delete()
                        context.log().i(TAG, "Cleaned up temporary cache file: ${file.name}")
                    } catch (e: Exception) {
                        context.log().e(TAG, "Failed to clean up cache file: ${file.name}")
                    }
                }
            }
        }.start()
